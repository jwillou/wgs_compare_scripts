...
#mt
pdf("bodysize_mt.pdf", height=4.25, width=4)
sink(file="bodysize_mt.txt")
plot(-100, -100, xlim=c(0, 300), ylim=c(-0.01, 0.32),xlab=NA, ylab="mitochondrial divergence rate", axes=F)
axis(side=1, at=seq(0,300,100), labels=T, tick=T, pos=-0.01)
axis(side=2, at=seq(0,0.30,0.1), labels=T, tick=T, pos=-10)
segments(x0=-10,x1=310,y0=-0.01, y1=-0.01)
segments(x0=-10,x1=-10,y0=-0.01, y1=0.32)
segments(x0=-10,x1=310,y0=0.32,  y1=0.32)
segments(x0=310,x1=310,y0=-0.01, y1=0.32)
#mammals
i = 1
t = data[data$class==classes[i],]
t = data.frame(gen_K80=t$mtW_K80, meanbd=t$meanbd)
t = t[complete.cases(t),]
tlm = lm(gen_K80~meanbd, data=t)
x0=min(t$meanbd, na.rm=T)
x1=max(t$meanbd, na.rm=T)
y0=(tlm$coefficients[2]*min(t$meanbd, na.rm=T)) + tlm$coefficients[1]
y1=(tlm$coefficients[2]*max(t$meanbd, na.rm=T)) + tlm$coefficients[1]
segments(x0=x0, x1=x1, y0=y0, y1=y1, col=colors[i], lwd=2)
points(x=t$meanbd, y=t$gen_K80, col=alpha(colors[i], 0.8), pch=16, cex=1)
print(classes[i])
print(summary(tlm))
sink()  
