setwd("/Users/jannawilloughby/GDrive/WGS_divergence/data/metabolic/")
library(scales)
library(vioplot)
library(phytools)
...
meta = read.table("mammalbmr.csv", header=T, sep=",")

#find unique values for bmr by species
spp = unique(meta$species)
o = NULL
for(s in 1:length(spp)){
  t = meta[meta$species==as.character(spp[s]),,drop=F]
  o = rbind(o, c(as.character(spp[s]), mean(t$BMR, na.rm=T)))
}
colnames(o) = c("species", "bmr")
meta = as.data.frame(o)
t = merge(data, meta, by.x="sp1", by.y="species", all.x=T)
colnames(t) = c(colnames(data), "sp1_bmr")
data$sp1_bmr = t$sp1_bmr
t = merge(data, meta, by.x="sp2", by.y="species", all.x=T)
colnames(t) = c(colnames(data), "sp2_bmr")
data$sp2_bmr = t$sp2_bmr

####total repetitive elements####
data$meanbmr = apply(cbind(data$sp1_bmr, data$sp2_bmr), 1, mean, na.rm=T)
for(r in 1:nrow(data)){
  if(is.na(data$meanbmr[r])){
    if(!is.na(data$sp1_bmr[r])){
      data$meanbmr[r] = data$sp1_bmr[r]
      next
    }
    if(!is.na(data$sp2_bmr[r])){
      data$meanbmr[r] = data$sp2_bmr[r]
      next
    }
  }
}

####DID NOT CONTINE, NOT ENOUGH DATA POINTS

colors  = c("firebrick2", "dodgerblue2",     "goldenrod3") #
classes = c("Mammalia",   "Actinopterygii",  "Aves") #  

sink(file="bmr_genomic.txt")
pdf("bmr_genomic.pdf", height=5.25, width=5)
...
t = data.frame(gen_K80=sqrt(t$gen_K80), meanbmr=t$meanbmr)
t = t[complete.cases(t),]
tlm = lm(gen_K80~meanbmr, data=t)
print(classes[i])
print(summary(tlm))
plotcurve = data.frame(x=t$meanbmr, y=predict(tlm))
